...
public class WiredEffectGiveUserFurni extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;

    protected String message = "";

    public WiredEffectGiveUserFurni(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserFurni(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0 &&
            ...
            if (habbo != null) {
                Integer itemId = Integer.valueOf(this.message);
                Item baseItem = Emulator.getGameEnvironment().getItemManager().getItem(itemId.intValue());
                if (baseItem == null) {
                    habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.givefurni.error").replace("%itemid%", itemId.toString()));
                    return true;
                }
                HabboItem newItem = Emulator.getGameEnvironment().getItemManager().createItem(habbo.getHabboInfo().getId(), baseItem, 0, 0, "");
                habbo.getInventory().getItemsComponent().addItem(newItem);
                habbo.getClient().sendResponse((MessageComposer)new AddHabboItemComposer(newItem));
                habbo.getClient().sendResponse((MessageComposer)new InventoryRefreshComposer());
                habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.givefurni.success").replace("%itemid%", itemId.toString()));
            }
        }
        return false;
    }
