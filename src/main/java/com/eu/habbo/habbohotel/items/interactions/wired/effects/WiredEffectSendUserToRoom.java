...
public class WiredEffectSendUserToRoom extends InteractionWiredEffect {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0 &&
            ...
            if (habbo != null) {
                Room toroom = Emulator.getGameEnvironment().getRoomManager().getRoom(Integer.valueOf(this.message).intValue());
                if (toroom == null) {
                    habbo.whisper(Emulator.getTexts().getValue("wired.effect.sendusertoroom.roomerror"), RoomChatMessageBubbles.ALERT);
                    return true;
                }
                if (toroom.getId() == habbo.getHabboInfo().getCurrentRoom().getId())
                    return true;
                habbo.goToRoom(toroom.getId());
                return true;
            }
        }
        return false;
