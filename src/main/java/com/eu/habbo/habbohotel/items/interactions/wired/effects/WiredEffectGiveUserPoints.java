...
public class WiredEffectGiveUserPoints extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;

    protected String message = "";

    public WiredEffectGiveUserPoints(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserPoints(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0) {
            ...
            if (roomUnit != null) {
                ...
                if (habbo != null) {
                    habbo.givePoints(Integer.valueOf(Emulator.getConfig().getValue("phbplugin.wireds.seasonal.id", "103")).intValue(), value.intValue());
                    habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.givediamonds.success").replace("%value%", value.toString()));
                }
            }
        }
        return false;
