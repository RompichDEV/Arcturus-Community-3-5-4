...
public class WiredEffectGiveUserAchievement extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;

    protected String message = "";

    public WiredEffectGiveUserAchievement(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserAchievement(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0) {
            ...
            if (roomUnit != null) {
                ...
                if (habbo != null &&
                        value.intValue() > 0) {
                    habbo.getHabboStats().addAchievementScore(value.intValue());
                    habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.giveachievement.success").replace("%value%", value.toString()));
                }
            }
        }
        return false;
