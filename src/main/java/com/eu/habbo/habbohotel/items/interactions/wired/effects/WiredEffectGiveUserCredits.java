...
public class WiredEffectGiveUserCredits extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;

    protected String message = "";

    public WiredEffectGiveUserCredits(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserCredits(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0) {
            ...
            if (roomUnit != null) {
                ...
                if (habbo != null) {
                    habbo.giveCredits(value.intValue());
                    habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.givecredits.success").replace("%value%", value.toString()));
                }
            }
        }
        return false;
