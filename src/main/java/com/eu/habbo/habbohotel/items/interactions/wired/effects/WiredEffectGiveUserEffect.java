...
public class WiredEffectGiveUserEffect extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;

    protected String message = "";

    public WiredEffectGiveUserEffect(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserEffect(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0 &&
            ...
            if (habbo != null) {
                Integer effect = Integer.valueOf(this.message);
                if (effect.intValue() > 0) {
                    habbo.getHabboInfo().getCurrentRoom().giveEffect(habbo, effect.intValue(), -1);
                    return true;
                }
            }
        }
