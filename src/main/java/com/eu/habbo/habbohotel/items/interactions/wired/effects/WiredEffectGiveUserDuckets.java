...
public class WiredEffectGiveUserDuckets extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;

    protected String message = "";

    public WiredEffectGiveUserDuckets(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserDuckets(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.message.length() > 0) {
            ...
            if (roomUnit != null) {
                ...
                if (habbo != null) {
                    habbo.givePoints(0, value.intValue());
                    habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.giveduckets.success").replace("%value%", value.toString()));
                }
            }
        }
        return false;
